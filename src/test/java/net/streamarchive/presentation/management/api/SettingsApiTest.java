...
class SettingsApiTest {
    ...
    private static String wrongJsonSettings;
    ...
    static void init() throws IOException {
        ...
        settings.setRecordStreamPath("/path/to/storage");
        settings.setServerPort(8080);
        settings.setDbUsername("jerfe");
        settings.setDbPassword("iaoejfvioe");
        wrongJsonSettings = mapper.writeValueAsString(settings);
    ...
    public void shouldReturnSettingsWhenCallGet() throws Exception {
        ...
        this.mockMvc.perform(get("/api/v1/settings")).andDo(print()).andExpect(status().isOk())
                ...
                , responseFields(
                        ...
                        fieldWithPath("userPass").description("recorder password").attributes(key("constrains")
                        ...
                        fieldWithPath("callbackAddress").description("Callback address for twitch webhook api (needed for stream events")
                                .attributes(key("constrains").value(constraintDescriptions.descriptionsForProperty("callbackAddress"))),
                        fieldWithPath("recordStreamPath").description("Path in server filesystem").attributes(key("constrains")
                                .value(constraintDescriptions.descriptionsForProperty("recordStreamPath"))),
                        fieldWithPath("serverPort").description("Recorder server port").attributes(key("constrains")
                                .value(constraintDescriptions.descriptionsForProperty("serverPort"))),
                        fieldWithPath("remoteDBAddress").description("Streamarchive db server address. Needed for publishing streams to streamarchive portal").attributes(key("constrains")
                                .value(constraintDescriptions.descriptionsForProperty("remoteDBAddress"))),
                        fieldWithPath("dbUsername").description("Streamarchive db server username").attributes(key("constrains")
                                .value(constraintDescriptions.descriptionsForProperty("dbUsername"))),
                        fieldWithPath("dbPassword").description("Streamarchive db server password").attributes(key("constrains")
                        ...
                        fieldWithPath("clientID").description("Twitch API client id (more on https://dev.twitch.tv/docs/v5")
                                .attributes(key("constrains").value(constraintDescriptions.descriptionsForProperty("clientID"))))));
    ...
    public void shouldReturn200WhenSaveSettings() throws Exception {
        ...
        this.mockMvc.perform(post("/api/v1/settings").contentType(MediaType.APPLICATION_JSON)
                ...
                .andDo(document("settings/{methodName}",
                        requestFields(fieldWithPath("user").description("recorder login").attributes(key("constraints")
                                        .value(constraintDescriptions.descriptionsForProperty("user"))),
                                fieldWithPath("userPass").description("recorder password").attributes(key("constrains")
                                        .value(constraintDescriptions.descriptionsForProperty("userPass"))),
                                fieldWithPath("callbackAddress").description("Callback address for twitch webhook api (needed for stream events")
